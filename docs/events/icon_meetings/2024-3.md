...
MJ welcomes everyone and shares the [latest news about ICON from C2SM :material-open-in-new:](https://polybox.ethz.ch/index.php/s/uINvAjV0N5LbLUQ){:target="_blank"}.
...
- [General Information for Users on Alps :material-open-in-new:](https://github.com/eth-cscs/UserLabDay/blob/master/2024/general_information_alps.pdf){:target="_blank"}
- [GitHub repo UseLabDay :material-open-in-new:](https://github.com/eth-cscs/UserLabDay/tree/master/2024){:target="_blank"}
...
BG mentions using Tödi and that while the instructions on C2SM initially worked, two weeks later the executables stopped working and the instructions disappeared. ML responds that there is now an Alps section on the landing page. It should be possible to do the same tasks as before, but without using absolute paths to his environment. Instead, tags can be used. The updated instructions are available on the [workshop materials page](https://c2sm.github.io/alps/#introductory-workshop-material) as well as the [user landing page](https://c2sm.github.io/alps/uenvs/#the-uenv-command-line-tool). ML encourages anybody to reach out to him on Slack if any issues arise.
...
### Will Sawyer
WS has been working on benchmarking ICON on the new system. Alps was officially inaugurated on Saturday. WS has benchmarked various versions of ICON and is preparing a presentation: one for this afternoon at EXCLAIM. They can share an early version of the presentation. Shares presentation (TODO Micha: add link)

### Matthias Kraushaar
MK is the service manager for climate and weather. Although his training is not related to weather and climate, he understands the challenges and is focused on finding solutions to keep the work moving forward. Alps is very beneficial for machine learning and will also become part of the climate and weather platform. MK will help introduce the new features and has created a [Slack space for users](https://cscs-users.slack.com/). A channel for the weather and climate platform will be set up, and while it will be monitored from the CSCS side, very technical issues should be turned into tickets.
